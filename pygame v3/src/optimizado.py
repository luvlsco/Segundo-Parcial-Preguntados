...
def configurar_juego():
    ...
    ventana_configuracion = pygame.image.load("assets/images icon/fondo_configurar.jpg") 
    ventana_configuracion = pygame.transform.scale(ventana_configuracion,(ANCHO, ALTO))
...
def ventana_agregar_pregunta():
    ...
    ventana_pregunta = pygame.transform.scale(ventana_pregunta, (ANCHO, ALTO))
    ...
    boton_volver_inicio = crear_boton(
        ...
        dimensiones=(50,50),posicion=(20, 20),
        funcion=lambda: volver_a_ventana_principal())
...
def ventana_agregar_preguntas():
    ...
    fondo = pygame.image.load("assets/images icon/fondo_configurar.jpg")
...
def agregar_preguntas(diccionario_preguntas):
    ...
    while True:
        ...
        ventana_configuracion = pygame.image.load("assets/images icon/fondo_agregar_pregunta.png") 
        ventana_configuracion = pygame.transform.scale(ventana_configuracion,(ANCHO, ALTO))
        ventana.blit(ventana_configuracion, (0, 0))
        ...
        texto_entrada = fuente.render(entrada, True, (0, 0, 0))  
...
def ventana_jugar():
    ...
    sonido_correcto = pygame.mixer.Sound("assets/sounds/sfx/correcto.mp3")
    sonido_incorrecto = pygame.mixer.Sound("assets/sounds/sfx/incorrecto.mp3")

    pantalla_jugar = pygame.image.load("assets/images icon/fondo_jugar.png")
    pantalla_jugar = pygame.transform.scale(pantalla_jugar, (ANCHO, ALTO))
...
def graficar_clasificacion(ventana, clasificacion, fuente, posicion_inicial):
    ...
    espacio_entre_lineas = fuente.get_height() + 10
    altura_total = len(clasificacion[:10]) * espacio_entre_lineas

    x_inicio = (800 - max([fuente.render(f"{indice + 1}. {jugador['nombre']} - {jugador['puntuacion']} puntos", True, (255, 255, 255)).get_width() for indice, jugador in enumerate(clasificacion)])) // 2
    y_inicio = (600 - altura_total) // 2
    ...
    for indice, jugador in enumerate(clasificacion[:10]):
        ...
        ventana.blit(texto_render, (x_inicio, y_inicio + indice * espacio_entre_lineas))
...
def ventana_top():
    ...
    ventana_top = pygame.transform.scale(ventana_top, (ANCHO, ALTO))
